...
#[dojo::contract]
pub mod actions {
    use super::{IActions, Artist, User, Tattoo};
    use starknet::{ContractAddress, get_caller_address};

    use dojo::model::{ModelStorage};
    use dojo::event::EventStorage;

    // use inkfinity::components::payable::PayableComponent;
    ...
    //     collection_factory: ContractAddress, 
    // }

    #[abi(embed_v0)]
    impl ActionsImpl of IActions<ContractState> {

        fn mint(ref self: ContractState, token_id: u256, uri: ByteArray, price: u256, collection: ContractAddress) {
            let artist: ContractAddress = get_caller_address();
        }

        fn invest(ref self: ContractState, amount: u256) {
            let artist: ContractAddress = get_caller_address();
        }

        fn create_drop(ref self: ContractState) {
            let artist: ContractAddress = get_caller_address();
        }
        ...
        fn change_owner(ref self: ContractState, token_id: u256, new_owner: ContractAddress) {
            let artist: ContractAddress = get_caller_address();
        }

        fn change_price(ref self: ContractState, token_id: u256, price: u256) {
            let artist: ContractAddress = get_caller_address();
        }
